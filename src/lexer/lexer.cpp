...
Lexer::Lexer(const Lexer &toCopy) {
    this-> states = toCopy.states;
    this->inputs = toCopy.inputs;
    this->lookUpTable = toCopy.lookUpTable;
    this->lineNumber = 0;
    this->charPosition = 0;
...
Lexer& Lexer::operator=(const Lexer &toAssign) {
    this-> states = toAssign.states;
    this->inputs = toAssign.inputs;
    this->lookUpTable = toAssign.lookUpTable;
    this->lineNumber = 0;
    this->charPosition = 0;
    this->fileName = toAssign.fileName;
    std::vector<std::string> outFiles = {fileName+".outlexerrors", fileName+".outlextokens"};
    this->logObserver =  std::make_unique<lexerLogObservers>(outFiles);
    this->logger = toAssign.logger;
    this->logger->subscribe(*logObserver);
    return *this;
}
...
while (token.isEmpty()){
    ...
    messageTemp.push_back(token.toString());
    messageBuilder message = {messageTemp, token.getIsError(),'L'};
    logger->log(message);
    return token;
}
